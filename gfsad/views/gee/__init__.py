...
def extract_info(ee=ee, lat=31.74292, lon=-110.051375, date_start='2008-01-01',
...
def get_segments():
    ...
    pt = ee.Geometry.Point(-83.54015, 44.40350)
    clip = pt.buffer(2000).bounds()
    # imageOriginal = ee.Image("UMD/hansen/global_forest_change_2013").clip(clip)
    collection = ee.ImageCollection('LANDSAT/LC8_L1T_8DAY_NDVI')

    imageOriginal = collection.max().clip(clip)
    imageOriginal.addBands(collection.min())
    imageOriginal.addBands(collection.median())
    imageOriginal.addBands(collection.mean())
    imageOriginal.addBands(collection.sum())

    imageOriginal.clip(clip)

    maxObjectSize = 40
    time.sleep(1)
    imageClustered = ee.apply("Test.Clustering.RegionGrow", {
        "image": imageOriginal,
        "threshold": 0.0005,
    ...
    print imageConsistent.getDownloadUrl()
    time.sleep(1)
...
def get_segments_more_bands(lat=41.01, lon=-96, bufferSize=2000, blurRadius=30):
    ...
    imageClustered = ee.apply("Test.Clustering.KMeans", {
        "image": image.select(
            ['ndvi_mean', 'panchromatic_mean', 'brightness_mean', 'greenness_mean','wetness_mean']),
    })
